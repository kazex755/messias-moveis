...
// Bloqueia cliques, seleção de texto e atalhos de sistema  
 ...
 integrityCheck: () => {  
     const authorized = ["localhost", "127.0.0.1", "GITHUB.IO", "SEUSITE.COM"];   
...
(function () {
    ...
    // ========================================================================
    // 3. MOTOR DE INTELIGÊNCIA ARTIFICIAL (IA DE DESCRIÇÃO) - ULTRA VARIEDADE
    // ========================================================================
    const IAMotor = {
        getIntro: name => {
            const intros = [
                `Apresentamos este magnífico ${name}, uma peça que redefine o conceito de sofisticação.`,
                `Este ${name} foi projetado para quem não abre mão da elegância e funcionalidade no dia a dia.`,
                `Descubra o equilíbrio perfeito entre design contemporâneo e marcenaria artesanal com este ${name}.`,
                `Elevando o padrão do seu ambiente: este ${name} une tecnologia de precisão com acabamento manual.`,
                `Otimize seu espaço com a exclusividade deste ${name}, desenvolvido sob os mais rígidos padrões de qualidade.`,
                `Sinta a textura e aprecie o design minimalista deste ${name}, feito para durar gerações.`,
                `Estética atemporal e engenharia de ponta se encontram neste exemplar único de ${name}.`
            ];
        ...
        getDetails: name => {
            ...
            if (
                ...
                const details = [
                    "Equipado com sistemas de amortecimento blumotion nas corrediças, garantindo fechamento suave mesmo com carga total. O MDF possui proteção Nano-Clean contra umidade.",
                    "Estrutura reforçada em 18mm com bordas seladas a laser, evitando infiltrações. Inclui divisores internos organizadores com acabamento em couro sintético ou alumínio.",
                    "Puxadores em perfil oculto de alumínio anodizado e dobradiças com click-on para regulagem milimétrica. Ideal para ambientes de alto tráfego e uso intenso."
                ];
                return details[Math.floor(Math.random() * details.length)];
            }

            // DORMITÓRIOS E GUARDA-ROUPAS
            if (
                n.includes("quarto") ||
                n.includes("guarda") ||
                n.includes("armário") ||
                ...
                const details = [
                    "Possui cabideiros com iluminação LED integrada e sensores de presença. O acabamento interno é feito em tecido aveludado, protegendo suas roupas mais delicadas.",
                    "Gavetões com extração total e sistema push-to-open (toque para abrir). A profundidade foi otimizada para maximizar o armazenamento sem comprometer a circulação.",
                    "Painéis externos com tecnologia anti-digital e texturas sincronizadas que imitam perfeitamente o toque da madeira natural maciça."
                ];
                return details[Math.floor(Math.random() * details.length)];
            }
            ...
            if (
                ...
                const details = [
                    "Desenvolvido com sistema de fiação inteligente oculta e passagens de cabos integradas. Estrutura robusta capaz de suportar telas de última geração com total segurança.",
                    "Design com nichos chanfrados em 45 graus, criando um efeito visual de peça única sem emendas aparentes. Acabamento em laca alto brilho ou fosco acetinado.",
                    "Integra prateleiras flutuantes com suporte invisível e vãos ventilados para evitar o superaquecimento de aparelhos eletrônicos e consoles."
                ];
                return details[Math.floor(Math.random() * details.length)];
            }

            // GERAL / PADRÃO
            return "Cada detalhe foi pensado para máxima durabilidade. Utilizamos ferragens de aço inoxidável com garantia de ciclo de abertura e bordas com selagem térmica de 2mm, protegendo o móvel contra impactos e descascamento.";
        },
        getConclusion: () => {
            const closings = [
                "Transforme sua casa hoje mesmo com a qualidade que só a nossa marcenaria pode oferecer.",
                "Disponível para personalização completa de cores, texturas e ferragens conforme o seu projeto.",
                "Uma escolha inteligente para quem busca durabilidade de décadas e um estilo que nunca sai de moda.",
                "Garanta este item exclusivo e leve o luxo da marcenaria de alto padrão para o seu cotidiano.",
                "Projeto assinado com foco em ergonomia e aproveitamento inteligente de cada centímetro disponível.",
                "Consulte nossa paleta de cores exclusivas para combinar perfeitamente com seu piso e decoração."
            ];
            return closings[Math.floor(Math.random() * closings.length)];
        },
        generateFullDescription: name => {
            return `${IAMotor.getIntro(name)} ${IAMotor.getDetails(name)} ${IAMotor.getConclusion()}`;
        }
    };
    ...
    const CartModule = {
        ...
        clear: () => {
            if (
                AppState.data.cart.length > 0 &&
                confirm("Remover todos os itens?")
            ) {
                AppState.data.cart = [];
                CartModule.sync();
                CartModule.renderSidebar();
   ...
   // ========================================================================
        ...
        send: () => {
            ...
            ContractModule.check(() => {
                try {
                    ...
                    // O link api.whatsapp.com é mais eficiente para "chamar" o app instalado
                    const whatsappUrl = `https://api.whatsapp.com/send?phone=${cleanPhone}&text=${encodeURIComponent(message)}`;

                    // No celular, o window.location.href costuma ser mais agressivo para abrir apps externos
    ...
    document.addEventListener("DOMContentLoaded", Module9.init);
...
// ========================================================================
// 📸 VISUAL CONSULTANCY ENGINE - UPLOAD DE AMBIENTE
// ========================================================================
const PhotoSystem = {
    inject: () => {
        const modalBody = document.querySelector("#app-modal > div");
        ...
        container.style.cssText = `
            margin: 15px 0; padding: 15px; border: 2px dashed rgba(128,128,128,0.3);
        ...
        container.innerHTML = `
            ...
            <div id="photo-label">
                ...
                <p style="font-size: 11px; margin: 5px 0 0 0; opacity: 0.8;">
                    Anexar foto do local ou desenho (Opcional)
                </p>
            </div>
        `;
        ...
        document
            ...
            ?.addEventListener("change", e => {
                ...
                if (file) {
                    ...
                    <span style="color:#27ae60; font-weight:bold;">✅ Foto selecionada!</span>
                    <p style="font-size:10px; opacity:0.6;">Ela será enviada no WhatsApp.</p>
                `;
                    // Salva o nome do arquivo para o WhatsApp saber que tem anexo
                    AppState.ui.hasPhoto = true;
                }
            });

        const btn = modalBody.querySelector("button");
        modalBody.insertBefore(container, btn);
    }
};

// Monitora a abertura do Modal para injetar o campo de foto
...
// ========================================================================
// 🎨 SMART COLOR & TEXTURE VALIDATOR v3.0 (AI HEURISTICS)
// ========================================================================
const ColorValidator = {
    // Base de Conhecimento Semântica (Simula a IA de padrões de marcenaria)
    keywords: [
        "BRANCO",
        "PRETO",
        "CINZA",
    ...
    /**
     * Validação Híbrida: Motor de Renderização + Busca Semântica
     */
    isValid: input => {
        const termo = input.toUpperCase().trim();
        if (termo.length < 3) return false;

        // 1. TESTE DE MOTOR (IA do Navegador): Verifica se é uma cor real (hex, rgb, ou nomeada)
        const tester = document.createElement("div");
        tester.style.color = termo;
        // Se o navegador aceitou a cor, ele a converte internamente. Se não, fica vazio.
        const isRealColor = tester.style.color !== "";

        // 2. BUSCA SEMÂNTICA: Verifica se o termo contém padrões de MDF/Madeira
        const isWoodPattern = ColorValidator.keywords.some(key =>
            termo.includes(key)
        );

        // 3. VALIDAÇÃO DE CONTEXTO: Impede sequências repetitivas (ex: "aaaaa", "123123")
        const isGarbage = /(.)\1{3,}/.test(termo) || /^[0-9]+$/.test(termo);

        return (isRealColor || isWoodPattern) && !isGarbage;
    },

    /**
     * Hook de Interceptação Inteligente
     */
    hook: () => {
        ...
        inputCor.oninput = () => {
            if (ColorValidator.isValid(inputCor.value)) {
                inputCor.style.borderColor = "#27ae60"; // Verde se válido
            } else {
        ...
        btnConfirm.onclick = e => {
            ...
            if (!ColorValidator.isValid(valor)) {
                ...
                NotificationSystem.show(
                    "Cor ou acabamento não reconhecido. Tente ex: Branco, Carvalho ou Grafite.",
                    "error"
                );
                return;
            }
...
// Monitor de Ativação (Observa o Modal de Personalização)
